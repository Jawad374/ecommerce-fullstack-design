    ...
    const fetchProduct = async () => {
      try {
        ...
        setFormData({
            ...
            newImages: [],
            // Ensure additional fields are mapped if they exist in DB
            type: data.type || '',
            ...
            brand: data.brand || '',
            status: data.status || (data.stock > 0 ? 'Active' : 'Out of Stock')
        });
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.keys(formData).forEach(key => {
        if (key !== 'existingImages' && key !== 'newImages' && formData[key] !== null) {
              ...
              <div className="p-6">
                 <div className="space-y-4">
                    ...
                    {formData.existingImages.length > 0 && (
                        <div>
                            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Current Images</p>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {formData.existingImages.map((img, index) => (
                                    <div key={`existing-${index}`} className="group relative aspect-square bg-gray-100 rounded-xl overflow-hidden border border-gray-200">
                    ...
                    {/* New Images Section */}
                     {imagePreviews.length > 0 && (
                        <div className={formData.existingImages.length > 0 ? "pt-4 border-t border-dashed border-gray-200" : ""}>
                             <p className="text-xs font-semibold text-blue-500 uppercase tracking-wider mb-3">New Uploads (Unsaved)</p>
                             <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {imagePreviews.map((preview, index) => (
                                    <div key={`new-${index}`} className="group relative aspect-square bg-blue-50 rounded-xl overflow-hidden border border-blue-200">
